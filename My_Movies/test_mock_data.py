...
def test_get_movies_empty_genre():
    """Test the get_movies endpoint with an empty genre ID."""
    url = "http://127.0.0.1:5080/get_movies?genre=&year=2023"
    ...
    try:
        ...
        if response.status_code == 400:
            logging.info("Correctly received 400 Bad Request for empty genre ID")
            return True
        else:
        ...
        logging.error(f"Error testing get_movies endpoint with empty genre: {e}")
        return False
...
def main():
    ...
    try:
        ...
        empty_genre_success = test_get_movies_empty_genre()
        ...
        logging.info(f"  Empty Genre Test: {'PASSED' if empty_genre_success else 'FAILED'}")
        
        if genres_success and movies_success and empty_genre_success:
